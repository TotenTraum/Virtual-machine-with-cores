    ...
    abstract class TransferCommand: ICommand
    {
                        ...
                        ref Stack stack, ref PSW_t psw, ref Memory mem)
        {
            ...
            if (data.arg1_state == Instruction.arg_states.reg)
                run(ref regs[data.arg1.Value()], ref Address, ref stack, ref mem);
            else if (data.arg1_state == Instruction.arg_states.mem)
            {
                register_t left_reg = 0;
                run(ref left_reg, ref Address, ref stack, ref mem);
        ...
        abstract protected void run(ref register_t out_reg, ref register_t? reg,
                                    ref Stack stack,ref Memory mem);
    }
    // address         = null <=> stdin 
    //                 >=   0 <=> адрес в памяти
    // stack[top]      =    0 <=> int
    ...
    class Write : TransferCommand
    {
                                    ...
                                    ref Stack stack, ref Memory mem)
        {
            ...
            if (address == null)
                ...
                {
                    case 0:
                        Console.Write(out_reg.integer);
                        break;
                    case 1:
                        Console.Write(out_reg.Float);
                        break;
                    case 2:
                        Console.WriteLine(out_reg.integer);
                        break;
                    case 3:
                        Console.WriteLine(out_reg.Float);
                        break;
                    default:
                        throw new Exception();
                }
            else if (address.integer >= 0)
                mem[(int)address.integer] = out_reg;
            else
                throw new Exception();
        }
    }
